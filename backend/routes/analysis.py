...
os.makedirs(app_config.REPORTS_FOLDER, exist_ok=True)

try:
    # import heavy analysis implementation
    from analysis_engine import run_full_analysis
except Exception as e:
    print(f"警告: 无法导入完整分析引擎: {e}")
...
def analyze_video_task(session_id, temp_video_path, report_folder_base):
    """实际执行视频分析任务的函数：调用完整分析引擎并将生成的文件放到报告目录下。"""
    ...
    try:
        # 如果完整引擎可用，则调用之生成真实报告
        if 'run_full_analysis' in globals():
            result = run_full_analysis(temp_video_path, report_folder_base, session_id)
            if result.get('error'):
                ANALYSIS_TASKS[session_id]['status'] = 'FAILED'
                ANALYSIS_TASKS[session_id]['result'] = {"error": result.get('error')}
            else:
                ANALYSIS_TASKS[session_id]['status'] = 'COMPLETED'
                # 将完整结果保存在任务中
                ANALYSIS_TASKS[session_id]['result'] = result
                # 补充常用 report_urls 以便前端直接访问
                if 'report_session_id' in result:
                    sid = result['report_session_id']
                    ANALYSIS_TASKS[session_id]['result'].setdefault('report_urls', {})
                    ANALYSIS_TASKS[session_id]['result']['report_urls'].update({
                        'report_html': f"/static/reports/{sid}/report.html",
                        'radar_chart': f"/static/reports/{sid}/radar_chart.png",
                        'line_chart': f"/static/reports/{sid}/line_chart.png",
                    })
                    # Debug: print generated report directory and files for troubleshooting
                    try:
                        output_dir = os.path.join(report_folder_base, sid)
                        print(f"分析完成，报告目录: {output_dir}")
                        ...
                        print(f"打印报告目录信息出错: {e}")
        else:
            # 回退到原来的模拟实现（保持兼容）
            time.sleep(5)
            raw_result = {
                "score": 8.5,
                "metrics": {"准确度": 92, "速度": 85, "稳定性": 88},
                "analysis_data": [
                    {"timestamp": 1000, "action": "直刺", "score": 9.0},
                    {"timestamp": 2000, "action": "弓步", "score": 8.2},
                    {"timestamp": 3000, "action": "防守", "score": 8.8}
                ]
            }
            ANALYSIS_TASKS[session_id]['status'] = 'COMPLETED'
            raw_result['report_urls'] = {
                "processed_video": f"/static/reports/{session_id}/processed_video.mp4",
                "radar_plot": f"/static/reports/{session_id}/tactical_radar_chart.png",
                "line_plot": f"/static/reports/{session_id}/posture_scores_timeline.png",
        ...
        ANALYSIS_TASKS[session_id]['status'] = 'FAILED'
        ANALYSIS_TASKS[session_id]['result'] = {"error": f"分析失败: {str(e)}"}
    finally:
        # 保留上传文件以便排错；如需清理可在这里删除 temp_video_path
        pass
...
def analyze_frame_endpoint():
    ...
    try:
        ...
        # 模拟分析结果
            ...
            # 同时提供一个更前端友好的 keypoints 字段（normalized coords 0..1），方便直接绘制
            try:
                ...
                for p in pose:
                    keypoints.append({
                        'x': float(p.get('x', 0)),
                        'y': float(p.get('y', 0)),
                        'confidence': float(p.get('visibility', 1.0)),
                        'detected': float(p.get('visibility', 0)) > 0.3
                    })
